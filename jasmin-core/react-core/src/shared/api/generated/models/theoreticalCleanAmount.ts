/**
 * Generated by orval v7.21.0 🍺
 * Do not edit manually.
 * Jasmin Platform API
 * CSA Management Platform API
 * OpenAPI spec version: 1.0.0
 */
import type { TheoreticalCleanAmountDayNumber } from './theoreticalCleanAmountDayNumber';
import type { UnitEnum } from './unitEnum';
import type { SizeVegetableEnum } from './sizeVegetableEnum';

/**
 * Mixin to automatically add related name fields.
 */
export interface TheoreticalCleanAmount {
  readonly id?: string;
  created_at?: string;
  /**
   * @minimum 0
   * @maximum 32767
   */
  year: number;
  /**
   * @minimum 1
   * @maximum 53
   */
  delivery_week: number;
  /**
   * @minimum 0
   * @maximum 32767
   * @nullable
   */
  day_number?: TheoreticalCleanAmountDayNumber;
  /**
   * @nullable
   * @pattern ^-?\d{0,8}(?:\.\d{0,2})?$
   */
  amount?: string | null;
  unit: UnitEnum;
  size?: SizeVegetableEnum;
  /** @nullable */
  note?: string | null;
  /** @nullable */
  created_by?: string | null;
  share_article: string;
  storage: string;
  /** @nullable */
  share_content?: string | null;
  /** @nullable */
  order_content?: string | null;
  readonly share_article_name?: string;
}
