 ...
 * renders. Don't call ``dayjs.extend(...)`` as a per-module load-time side
 * effect: plugin availability would then depend on *which chunk happened to
 * load first*, and in a production build a lazily loaded route chunk could
 * mount a widget that needs ``isoWeek`` / ``customParseFormat`` before the
 * plugin was registered — crashing AntD's ``DatePicker`` ("can't access
 * property 'date'").
 *
