...
const smallAndMediumVariations = (
  countS: number,
  countM: number,
...
describe("suggestPerShareAmounts", () => {
  it("scales each size with its weight (mirror: {S:5.0, M:10.0})", () => {
    const r = suggestPerShareAmounts(100, 1, smallAndMediumVariations(10, 5));
  ...
  it("floors a fractional split to the tenth (mirror: {S:3.3, M:6.6})", () => {
    const r = suggestPerShareAmounts(100, 1, smallAndMediumVariations(10, 10));
  ...
  it("divides the total across the weeks (2 weeks halves per-share vs 1 week)", () => {
    const oneWeek = suggestPerShareAmounts(100, 1, smallAndMediumVariations(10, 5));
    const twoWeeks = suggestPerShareAmounts(200, 2, smallAndMediumVariations(10, 5));
  ...
  it("is not distributable with zero subscribers", () => {
    const r = suggestPerShareAmounts(100, 1, smallAndMediumVariations(0, 0));
  ...
  it("is not distributable with no target, zero or negative weeks", () => {
    expect(suggestPerShareAmounts(0, 1, smallAndMediumVariations(10, 5)).distributable).toBe(false);
    expect(suggestPerShareAmounts(100, 0, smallAndMediumVariations(10, 5)).distributable).toBe(false);
    expect(suggestPerShareAmounts(-5, 1, smallAndMediumVariations(10, 5)).distributable).toBe(false);
  });

  it("floors to whole units for piece-based units (floorStep=1)", () => {
    // PCS: you can't deliver 0.3 of a lettuce — whole pieces only.
    const r = suggestPerShareAmounts(100, 1, smallAndMediumVariations(10, 10), { floorStep: 1 });
