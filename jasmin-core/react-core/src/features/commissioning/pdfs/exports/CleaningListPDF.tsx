...
const CleaningListPDF = ({ data, year: _year, week, dayName, t }: CleaningListPDFProps) => {
  return (
    <Document>
      <Page size="A4" style={listStyles.page}>
        <ListPDFHeader pill={t("commissioning.cleaning_list")}>
        ...
        <View style={listStyles.table}>
          ...
          {data.map((item, index) => (
            ...
            >
              ...
              <View style={[listStyles.cell, styles.colAmount, listStyles.cellCenter]}>
                <Text style={{ fontWeight: 700 }}>{item.computed_total_clean_amount_text || ""}</Text>
              </View>
              <View style={[listStyles.cell, listStyles.colNote, listStyles.cellLeft]}>
                <Text>{item.note || ""}</Text>
              </View>
