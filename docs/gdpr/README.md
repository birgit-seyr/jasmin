...
| **Art. 13, 14** | Information given to data subjects (privacy policy) | *Privacy notice — maintained per tenant, outside this repository* |
...
| **Art. 25** | Data protection by design + by default | (cross-cutting — see the internal auth reference and logging overview) |
...
| **Art. 32** | Security of processing | *Security documentation is maintained internally.* |
...
Documents in this directory are **public**. They may link only to other public
paths — committed docs in this directory, or source files under `jasmin-core/`.
Never link into `docs/code_audit/`, `docs/todos/` or `docs/security/`: those are
gitignored and resolve to nothing for anyone who clones the repository.
